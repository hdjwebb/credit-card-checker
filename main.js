...
const validateCred = arr => {
  ...
  for ( i = 0; i < reversedArray.length; i++) {         // loop through array
    ...
    const reducer = (accumulator, curr) => accumulator + curr;      // sum numbers in array
    return (newArray.reduce(reducer) + removedLast) % 10 === 0;     //true or false
}


// find invalid cards and return an array of them

const findInvalidCards = batchArray => {
  const invalidArray = [];                     						// new empty array
  for (let i in batchArray ) {                 						// loop through batchArray
    let currCard = batchArray[i];          
    if (!validateCred(currCard)) {             						// check if false
      invalidArray.push(currCard);             						// add to invalidArray 
    } 
  }
  console.log("Invalid Cards")
  return invalidArray;                         						// return the array
}


// extra function to check valid and invalid card and return both arrays.
